...
function Cart({ cart, updateCart }) {
	...
	return isOpen ? (
		<aside className='cbs-cart'>
			<button	className='cbs-cart-toggle-button' onClick={() => setIsOpen(false)} >
				Fermer
			</button>
			{cart.length > 0 ? (
				<div>
					...
					<button className='vider-panier-cta' onClick={() => updateCart([])}>Vider le panier</button>
					<h3>Total : {total} €</h3>
				</div>
			) : (
		...
		<div className='cbs-cart-closed'>
			<button	className='cbs-cart-toggle-button' onClick={() => setIsOpen(true)} >
				Ouvrir le Panier
			</button>
		</div>
	)
